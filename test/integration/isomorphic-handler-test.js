  ...
  it("Renders the page if the route matches", function(done) {
    const app = createApp(
      (pageType, params, config, client, { host }) =>
        Promise.resolve({ pageType, data: { text: "foobar", host: host } }),
    ...
    supertest(app)
      ...
      .then(res => {
        ...
        assert.equal(
          '<div data-page-type="home-page" data-reactroot="">foobar</div>',
          response.content
        );
  ...
  it("Throws a 404 if the route is not matched", function(done) {
    ...
    supertest(app)
      ...
      .then(res => {
        const response = JSON.parse(res.text);
        assert.equal(
          '<div data-page-type="not-found" data-reactroot="">foobar</div>',
          response.content
        );
